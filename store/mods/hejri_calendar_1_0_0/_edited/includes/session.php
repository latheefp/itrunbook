	...
	*/
	function format_date($gmepoch, $format = false, $forcedate = false)
	{
		...
		if ($date_cache[$format]['is_short'] !== false && !$forcedate && !($gmepoch < $midnight - 86400 || $gmepoch > $midnight + 172800))
		{
			...
			if ($day !== false)
			{
				return str_replace('||', $this->lang['datetime'][$day], strtr(@hdate($date_cache[$format]['format_short'], $gmepoch + $zone_offset), $date_cache[$format]['lang']));
			}
		}

		return strtr(@hdate($date_cache[$format]['format_long'], $gmepoch + $zone_offset), $date_cache[$format]['lang']);
	}
